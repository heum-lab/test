...
export function PlacePage() {
  ...
  return (
    <div>
      <PageHeader
        title="플레이스"
        description="네이버 플레이스 키워드 순위 관리"
        actions={
          <>
            <ExcelActions
              moduleKey="place"
              exportItems={items as unknown as Array<Record<string, unknown>>}
              onUploaded={() => qc.invalidateQueries({ queryKey: ['place'] })}
            />
            <Button
              onClick={() => {
                setEditing(null);
      ...
      <PlaceModal open={modalOpen} onOpenChange={setModalOpen} item={editing} />
    </div>
  );
}
