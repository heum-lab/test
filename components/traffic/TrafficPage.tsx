...
export function TrafficPage() {
  ...
  const [editing, setEditing] = useState<TrafficItem | null>(null);
  ...
  const { data, isLoading } = useTrafficList(queryParams);
  const items = data?.data ?? [];
  const total = data?.total ?? 0;

  const create = useCreateTraffic();
  const update = useUpdateTraffic();
  const del = useDeleteTraffic();
  const bulk = useBulkTraffic();
  ...
  return (
    <div>
      <PageHeader
        title="유입플"
        description="유입 전문 플랫폼 작업 관리"
        actions={
          <>
            <ExcelActions
              moduleKey="traffic"
              exportItems={items as unknown as Array<Record<string, unknown>>}
              onUploaded={() => qc.invalidateQueries({ queryKey: ['traffic'] })}
            />
            <Button
              onClick={() => {
                setEditing(null);
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>유입플 {editing ? '수정' : '등록'}</DialogTitle>
            <DialogDescription>
