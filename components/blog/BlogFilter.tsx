...
export type BlogFilterState = {
  agency_id: string;
  seller_id: string;
  status: (typeof BLOG_STATUSES)[number] | 'all';
  date_type: DateType;
  start: string;
  end: string;
  search: string;
...
export function BlogFilter({ value, onChange, onApply, onReset }: Props) {
  ...
  const set = (patch: Partial<BlogFilterState>) => onChange({ ...value, ...patch });
  ...
  return (
    <div className="mb-4 space-y-3 rounded-md border border-[var(--color-border)] bg-[var(--color-card)] p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          value={value.status}
          onValueChange={(v) => set({ status: v as BlogFilterState['status'] })}
        >
