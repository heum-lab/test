...
const ALLOWED_DATE_COLS = ['expose_start_date', 'guarantee_end_date'];
...
export async function GET(request: Request) {
  ...
  const dateType = searchParams.get('date_type') ?? 'expose_start_date';
  const start = searchParams.get('start');
  const end = searchParams.get('end');
  const search = searchParams.get('search')?.trim();
  const page = Math.max(1, Number(searchParams.get('page') ?? '1'));
  const pageSize = Math.min(150, Math.max(1, Number(searchParams.get('page_size') ?? '50')));
  const sort = searchParams.get('sort') ?? 'expose_start_date';
  ...
  if (start && ALLOWED_DATE_COLS.includes(dateType)) query = query.gte(dateType, start);
  if (end && ALLOWED_DATE_COLS.includes(dateType)) query = query.lte(dateType, end);
